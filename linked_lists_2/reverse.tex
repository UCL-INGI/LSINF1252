...
\newcommand{\oldconnectlist}[2]{%
  \draw[dashed, *->, color=gray] let \p1 = (#1.two), \p2 = (#1.center) in
    (\x1, \y2) -- (#2)}

\newcommand{\newconnectlist}[2]{%
  \path[->, color=red] let \p1 = (#1.two), \p2 = (#1.center) in (\x1, \y2)
  edge[bend right=90] (#2.north east)}
...
\newcommand{\newconnectpointer}[2]{%
  \draw[->, color=red] (#1) edge[out=0, in=-90] (#2)}
...
\begin{tikzpicture}[
  ...
  {[start chain]
    ...
    \node[below of=n1] (list) {\texttt{list}};
  ...
  {[start chain]
    \node[list, on chain, below of=after] (n1) {1};
    \node[list, on chain] (n2) {2};
    \node[list, on chain] (n3) {3};
    \node[list, on chain] (n4) {4};
    \node[list, on chain] (n5) {5};
    \node[on chain] (nil-right) {\texttt{NULL}};
    \node[left of=n1] (nil-left) {\texttt{NULL}};
    \node[below of=n1] (list) {\texttt{list}};

    \oldconnectpointer{list}{n1};
    \newconnectpointer{list}{n5};

    \oldconnectlist{n1}{n2};
    \oldconnectlist{n2}{n3};
    \oldconnectlist{n3}{n4};
    \oldconnectlist{n4}{n5};
    \oldconnectlist{n5}{nil-right};

    \path[->, color=red] let \p1 = (n1.two), \p2 = (n1.center) in
      (\x1, \y2) edge[bend right=90] (nil-left.north);
    \newconnectlist{n2}{n1};
    \newconnectlist{n3}{n2};
    \newconnectlist{n4}{n3};
    \newconnectlist{n5}{n4};}
