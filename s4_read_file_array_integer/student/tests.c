...
void test_open_fail() {
    set_test_metadata("q1", _("Return -1 when open fail"), 1);
...
void test_open_ok() {
    set_test_metadata("q1", _("Calling open only once"), 1);
    //Test when open() returns a correct file descriptor.
    gen_file(13);
    monitored.open = true;
    ...
    if(stats.open.last_return <= 2){
        push_info_msg(_("When open() works, your code returns -1."));
        CU_FAIL("");
    }
